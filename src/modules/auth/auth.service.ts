...
import { sendResetEmail } from "./auth.mail.service";
...
async forgotPassword(email: string) {
  ...
  // security: kullanıcı yoksa bile sessizce dön
  if (!user) return;

  // eski tokenları iptal et (opsiyonel ama iyi)
  ...
  // ekibin yazdığı contract'a uyuyor:
  const link = `${frontend}/login/forgot-password?token=${token}`;
...
async resetPassword(token: string, newPassword: string) {
  ...
  // makeResetToken ile aynı şekilde hash'le
